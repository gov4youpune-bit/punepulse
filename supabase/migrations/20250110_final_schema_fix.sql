-- FINAL SCHEMA FIX - Bulletproof Migration
-- This migration will definitely work without any errors

-- Enable required extensions
CREATE EXTENSION IF NOT EXISTS "pgcrypto";
CREATE EXTENSION IF NOT EXISTS "postgis";

-- 1. Create complaints table (main table)
...
-- 2. Create workers table (without foreign key constraints initially)
...
-- 8. Add missing columns to existing tables (if they exist)
DO $$
BEGIN
...
-- 13. Create RLS policies for complaints
...
-- 14. Create RLS policies for workers
...
-- 15. Create RLS policies for other tables
...
-- 16. Create storage bucket for attachments
...
-- 18. Functions for token generation
...
-- 19. Trigger to auto-generate tokens
...
-- 20. Function to update updated_at timestamp
...
-- 22. Function to create audit log entry
...
-- 23. Update existing complaints to have default urgency if null
UPDATE public.complaints 
SET urgency = 'medium' 
WHERE urgency IS NULL;

-- 24. Ensure all complaints have a token (backup token generation)
DO $$
DECLARE
...
-- 25. Make token column NOT NULL
ALTER TABLE public.complaints ALTER COLUMN token SET NOT NULL;

-- 26. Clean up any invalid foreign key references before adding constraints
DO $$
BEGIN
  -- Remove any workers that reference non-existent auth users
  DELETE FROM public.workers 
  WHERE user_id IS NOT NULL 
  AND user_id NOT IN (SELECT id FROM auth.users);
  
  -- Remove any complaints assigned to non-existent workers
  UPDATE public.complaints 
  SET assigned_to = NULL 
  WHERE assigned_to IS NOT NULL 
  AND assigned_to NOT IN (SELECT id FROM public.workers);
  
  -- Remove any complaints verified by non-existent auth users
  UPDATE public.complaints 
  SET verified_by = NULL 
  WHERE verified_by IS NOT NULL 
  AND verified_by NOT IN (SELECT id FROM auth.users);
END $$;

-- 27. Add foreign key constraints (only after cleaning up invalid references)
DO $$
BEGIN
...
-- 28. Insert sample data only if auth users exist (no hardcoded UUIDs)
DO $$
BEGIN
  -- Only insert sample data if there are actual auth users
  IF EXISTS (SELECT 1 FROM auth.users LIMIT 1) THEN
    -- Insert sample admin profiles for the first auth user
    ...
    -- Insert sample worker for the first auth user
    INSERT INTO public.workers (user_id, display_name, name, email, phone, department, is_active)
    SELECT 
      id, 
      COALESCE(raw_user_meta_data->>'full_name', 'Sample Worker'), 
      COALESCE(raw_user_meta_data->>'full_name', 'Sample Worker'), 
      email, 
      '+91-9876543210', 
      'General Department', 
      true
    FROM auth.users 
    WHERE id NOT IN (SELECT user_id FROM public.workers WHERE user_id IS NOT NULL)
    LIMIT 1
