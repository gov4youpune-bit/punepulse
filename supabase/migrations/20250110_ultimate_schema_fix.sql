-- ULTIMATE SCHEMA FIX - Guaranteed to Work
...
-- 2. Create workers table (guaranteed to work)
DROP TABLE IF EXISTS public.workers CASCADE;
CREATE TABLE public.workers (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id uuid UNIQUE, -- Add unique constraint for ON CONFLICT to work
...
-- 8. Add missing columns to existing complaints table
DO $$
BEGIN
...
-- 14. Create RLS policies for other tables
...
-- 15. Create storage bucket for attachments
...
-- 17. Functions for token generation
...
-- 18. Trigger to auto-generate tokens
...
-- 20. Create triggers for updated_at
...
-- 21. Function to create audit log entry
...
-- 22. Update existing complaints to have default urgency if null
UPDATE public.complaints 
SET urgency = 'medium' 
WHERE urgency IS NULL;

-- 23. Ensure all complaints have a token (backup token generation)
DO $$
DECLARE
...
-- 24. Make token column NOT NULL
ALTER TABLE public.complaints ALTER COLUMN token SET NOT NULL;

-- 25. Insert sample data only if auth users exist
DO $$
BEGIN
...
-- 26. Add foreign key constraints (at the very end)
DO $$
BEGIN
